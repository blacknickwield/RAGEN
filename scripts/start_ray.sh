...
# start_ray.sh
# Usage: bash scripts/start_ray.sh <train_script.sh> [args...]
...
if [[ "$NNODES" -le 1 ]]; then
    ...
    echo "Single node detected, running $COMMAND directly..."
    "$COMMAND" "$@"
else
    ...
    if [[ "$NODE_RANK" == "0" ]]; then
        ...
        ray start --head --node-ip-address="$MASTER_ADDR" \
                  ...
                  --disable-usage-stats \
                  --port="$MASTER_PORT" \
                  --min-worker-port=23000 \
                  --max-worker-port=27000
        ...
        echo "Submitting training job: $COMMAND ${ESCAPED_ARGS[*]}"
        ray job submit --address="[$MASTER_ADDR]:$MASTER_PORT" \
            --runtime-env-json="$RUNTIME_ENV_JSON" \
            -- "$COMMAND" "${ESCAPED_ARGS[@]}"
        exit_code=$?
        echo "Training job exited with code $exit_code, stopping Ray head..."
        exit "$exit_code"
    else
        # Worker node
        ...
        while kill -0 "$ray_worker_pid" >/dev/null 2>&1; do
            if ! ray status --address="[$MASTER_ADDR]:$MASTER_PORT" &>/dev/null; then
                echo "Ray head is unavailable, stopping local Ray worker..."
                head_lost=true
                ray stop --force || true
                break
            fi
        ...
        wait "$ray_worker_pid"
        ray_worker_exit_code=$?

        if [[ "$head_lost" == "true" ]]; then
            echo "Worker exited after Ray head shutdown"
            exit 0
        fi

        exit "$ray_worker_exit_code"
    fi
fi
